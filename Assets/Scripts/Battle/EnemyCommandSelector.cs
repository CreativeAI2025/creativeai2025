...
public class EnemyCommandSelector : MonoBehaviour
{
    ...
    {
        Logger.Instance.Log($"CheckConditionが呼ばれました。");
        bool match = false;
        foreach (var conditionRecord in record.enemyConditionRecords)
        {
            switch (conditionRecord.conditionCategory)
            {
                case ConditionCategory.Turn:
                    ...
                    Logger.Instance.Log($"CheckTurnConditionの結果 : {match}");
                    break;
                case ConditionCategory.HpRate:
                    match = CheckHpRateCondition(conditionRecord, enemyBattleId);
                    Logger.Instance.Log($"CheckHpRateConditionの結果 : {match}");
                    break;
            }
        }
        return match;
    ...
    {
        var enemyStatus = EnemyStatusManager.Instance.GetEnemyStatusByBattleId(enemyBattleId);
        int currentHp = enemyStatus.currentHp;
        int maxHp = enemyStatus.enemyData.HP;
        float hpRate = currentHp * 100f / maxHp;
        Logger.Instance.Log($"HP残量の条件を確認します。 currentHp : {currentHp} || maxHp : {maxHp} || HP残量 : {hpRate}");
    ...
    bool CanUseMagic(EnemyActionRecord record, int enemyBattleId)
    {
        ...
        int mpCost = record.skillData.cost;
        bool canUse = currentMp >= mpCost;
        Logger.Instance.Log($"{record.skillData.skillName} の魔法が使用できるか確認します。 currentMp : {currentMp} mpCost : {mpCost} canUse : {canUse}");
        return canUse;
    }
