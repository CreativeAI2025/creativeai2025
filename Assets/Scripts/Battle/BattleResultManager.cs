...
public class BattleResultManager : MonoBehaviour
{
    ...
    public void OnWin()
    {
        ...
        CharacterStatusManager.Instance.IncreaseExp(totalExp);
        CharacterStatusManager.Instance.IncreaseGold(gold);
    ...
    IEnumerator WinMessageProcess(int exp, int gold)
    {
        ...
        var firstMemberId = CharacterStatusManager.Instance.partyCharacter[0];
        var characterName = CharacterDataManager.Instance.GetCharacterName(firstMemberId);
        ...
        foreach (var id in CharacterStatusManager.Instance.partyCharacter)
        {
            var isLevelUp = CharacterStatusManager.Instance.CheckLevelUp(id);
            if (isLevelUp)
            {
                _pauseMessage = true;
                var characterStatus = CharacterStatusManager.Instance.GetCharacterStatusById(id);
                var level = characterStatus.level;
                characterName = CharacterDataManager.Instance.GetCharacterName(id);
