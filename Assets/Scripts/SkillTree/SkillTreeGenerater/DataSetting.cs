...
public class DataSetting : MonoBehaviour{
    ...
    //int cols = 11;//列
    int rows;//行
    [SerializeField]int cellSize = 75;//行間距離
    ...
    public void NodeLimitData() {//各階層でノードの個数の制限
    ...
    public void lineLimitData() {//階層のノード数に対して枝数の確率(最高枝数６)
    ...
    public void SkillOrStatusData() {//入力に対して次がスキルまたステータスの確率(スキル、ステータス、初期状態)
    ...
    public void SkillData() {//ユーザが触るのはここだけ
    ...
    public void NodeDataSet() {
        ...
        if (nodeSum > id) {
            for (int y = 0; y < rows; y++) {
                for (int x = 0; x < nodelimitPerRow[y]; x++) {
                    float drawPosX = x * cellSize - nodelimitPerRow[y] * cellSize / 2;//x座標
                    float drawPosY = y * cellSize - ((cellSize * ((float)rows)) / 2.0f) + cellSize / 2.0f;//y座標
