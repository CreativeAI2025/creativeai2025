...
public class SkillTreeGanerate1 : MonoBehaviour
{
    ...
    void View()
    {
        ...
        dataSetting1.TagSet();
        dataSetting1.SkillDataSet();
        dataSetting1.StatusDataSet();

        DrawNodes();//ノードの表示
        //DrawLines();//ノード間の線の描写
        StartCoroutine(WaitAndDrawLines());

        foreach (int[] c in dataSetting1.connections)
        {
            //Debug.Log(c[0] + "," + c[1]);
        }
    ...
    {
        int id = 0;
        Dictionary<int, string> tagData = dataSetting1.getTagData();//スキル・ステータスのタグを格納
        ...
        foreach (Node n in dataSetting1.nodeData)
        {
            ...
            {
                foreach (var data in dataSetting1.nodeSkillData)
                {
    ...
    bool hasNodeWithZeroInput()
    {
        ...
        foreach (int[] pair in dataSetting1.connections)
        {
        ...
        endList.Sort();
        //Debug.Log(dataSetting1.getNodeSum());
        ...
        for (int i = 1; i < dataSetting1.getNodeSum(); i++)
        {
    ...
    int SumSP()
    {
        ...
        foreach (var data in dataSetting1.nodeStatusData)
        {
            sum += data.GetSp();
        }
